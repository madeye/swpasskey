...
- **macOS signed `.app` spike: done (2026-09-07), HID half blocked by Apple.** `cmake --preset app` (static OpenSSL, no TPM) + `packaging/macos/make_app.sh` with the team's Developer ID identity and a Developer ID profile for `io.github.swpasskey.daemon` produce a bundle that depends only on system libraries and passes `codesign --verify --strict`. Results on macOS 26 (Apple silicon):
  ...
  - **Secure Enclave works in the signed bundle:** with the HID key stripped and `SWPASSKEY_HID_SOCKET` set, `--key-backend=auto` selects `se`, `tests/e2e/pyfido2_e2e.py` prints ALL OK, and a makeCredential/getAssertion round-trip leaves a row with `backend=se` in `swpasskeyctl list`. The **macOS Keychain DEK** path was exercised too (service `io.github.swpasskey`, account `dek`, no `--dek-file`).
