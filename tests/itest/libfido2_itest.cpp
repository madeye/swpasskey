...
// Kills the daemon even when a REQUIRE aborts the test case.
struct DaemonGuard {
  pid_t pid{-1};
  ~DaemonGuard() {
    if (pid > 0) {
...
TEST_CASE("fido2-token sees swpasskeyd over UHID", "[itest]") {
  ...
  REQUIRE(pid >= 0);
  if (pid == 0) {
    ::execl(bin.c_str(), bin.c_str(), "--testing", "--dek-file", "--key-backend=software", "--store",
            store.c_str(), nullptr);
    ::_exit(127);
  }
  ...
  INFO(info);
  REQUIRE(info.find("FIDO_2_1") != std::string::npos);
  REQUIRE(info.find("FIDO_2_0") != std::string::npos);
  REQUIRE(info.find("hmac-secret") != std::string::npos);
